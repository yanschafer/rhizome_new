...
<template>
  <HeaderComponent />
  <router-view />
  <section class="section flex flex-row justify-center gap-10" v-if="form">
    <div class="animate__animated animate__fadeIn container form-container">
      <div class="flex flex-row gap-5">
        <div class="flex flex-col col-auto justify-center align-center">
          <a class="arrow-wrapper" @click="goBack"><IconArrowLeft size="2rem" color="#3987da" /></a>
        </div>
      ...
      <div class="container-fluid flex flex-col gap-5 main-container">
        <label :class="{'error-label': fieldErrors.name}" class="input input-bordered flex items-center gap-2">
          ...
          <input v-model="reqData.name" type="text" class="grow input-primary" :placeholder='localizeDefault("fullNamePlaceholder")' />
        </label>
        ...
        <div class="label flex flex-row justify-start gap-2 button-row">
          <span class="label-text text-white">{{ localizeDefault("preferredContactMethod") }}</span>
...
@media only screen and (max-width: 600px) {
  ...
  .section {
    padding: 2rem 0.3rem 0.3rem 0.3rem !important;
  }
  .button-row {
    display: flex;
